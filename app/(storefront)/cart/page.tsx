...
export default function CartPage() {
  ...
  // Delivery isn't priced here — it depends on the address, which checkout collects.
  const message =
    `Hello ${site.name}! 🌸\n\nI'd like to order:\n` +
    detailed
      ...
      <div className="grid gap-10 lg:grid-cols-[1fr_360px]">
        ...
        <aside className="h-fit rounded-2xl border border-mist-200 bg-white p-6">
          ...
          <div className="mt-4 flex items-center justify-between border-t border-mist-200 pt-4">
            <span className="text-lg font-bold text-denim-800">Subtotal</span>
            <span className="font-serif text-2xl font-bold text-gradient">
              {formatINR(subtotal)}
            </span>
          </div>
          <p className="mt-1 text-xs text-neutral-500">
            Delivery charges depend on your PIN code and are shown at checkout.
          </p>
